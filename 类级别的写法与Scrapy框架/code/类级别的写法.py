...
class Parser():
    def do_parse(self, html):
        e = etree.HTML(html)
        contents = [div.xpath("string(.)").strip()
                    for div in e.xpath("//div[@class='content']")]
        urls = ["https://www.qiushibaike.com{}".format(
            url) for url in e.xpath("//ul[@class='pagination']/li/a/@href")]
